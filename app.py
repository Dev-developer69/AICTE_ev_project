...
# === Load model ===
model = joblib.load('code/forecasting_ev_model.pkl')
...
# Image
st.image("code/ev-car-factory.jpg")
...
@st.cache_data
def load_data():
    df = pd.read_csv("code/preprocessed_ev_data.csv")
...
for i in range(1, forecast_horizon + 1):
    ...
    recent_historical_cumulative = list(county_df['Electric Vehicle (EV) Total'].cumsum().values[-6:]) #Fixed historical cumulative data
